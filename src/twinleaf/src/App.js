...
function Twinleaf() {
  ...
  let [ canvasDims, setCanvasDims ] = useState([200, 200])
  ...
  useEffect(() => {
    ...
    setVariables(findVariablesInText(DEFAULT_SHADER_TEXT, []))
  }, [])

  return (<>
    <nav>
      <img src={leaf} style={{width: '50px'}} alt="" /> Twinleaf
        ...
        <div>
          ...
          {images.map((img, i) => (
              <div className={"img-thumbnail" + (img == null ? " no-image" : "")} key={"thumb_" + i}>
                  {img != null ? <img src={images[i].src} alt="" /> : <img src={upload} alt="Upload a file" />}
          ...
          <div className="shader-canvas-container">
            <ResizableBox
             width={200}
             height={200}
             onResize={(e, data) => { setCanvasDims([data.size.width, data.size.height]); }}
            >
              <ShaderCanvas
              frag={editorText}
              images={images}
